  ...
  async getRecommendationsByCategory(category: RecommendCategory): Promise<RecommendedTopic[]> {
    try {
      ...
      const prompt = `
        ...
        3. **블로그 유입 우선 (NEW)**: 네이버 블로그 데이터에서 등장하는 키워드 주제는 실제 독자들이 검색하는 관심사이므로, 해당 흐름을 뉴스 이슈와 결합하여 '정말 사람들이 찾는' 주제를 만들어내세요.
        4. **다양성 (CRITICAL)**: 사용자가 매번 검색할 때마다 완전히 새로운 시각의 주제를 원합니다! 이전 추천과 겹치거나 뻔한 주제를 피하고, 항상 [창의적이고 다채로운 앵글]로 10가지 전혀 다른 차별화된 키워드를 즉석에서 새롭게 기획하세요. (Random Seed: ${Math.random().toString(36).substring(7)})
        5. **화제성**: 현재 사람들이 가장 궁금해하고 검색량이 급증하는 주제여야 합니다.
        6. **차별화**: 뻔한 내용이 아니라, 블로거가 자신만의 시각을 더해 상위 노출될 수 있는 '전략적 키워드'로 가공하세요.
        7. **구체성**: 키워드는 블로그 제목으로 바로 써도 될 만큼 구체적이어야 합니다.
        8. **이유 명시**: 왜 이 주제를 오늘 써야 하는지(예: 정부 발표 직후, 커뮤니티 난리 남 등)를 짧고 강렬하게 적으세요.
