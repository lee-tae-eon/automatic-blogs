  ...
  async getRecommendationsByCategory(category: RecommendCategory): Promise<RecommendedTopic[]> {
    try {
      ...
      const prompt = `
        ...
        3. **다양성 (CRITICAL)**: 사용자가 매번 검색할 때마다 완전히 새로운 시각의 주제를 원합니다! 이전 추천과 겹치거나 뻔한 주제를 피하고, 항상 [창의적이고 다채로운 앵글]로 10가지 전혀 다른 차별화된 키워드를 즉석에서 새롭게 기획하세요. (Random Seed: ${Math.random().toString(36).substring(7)})
        4. **화제성**: 현재 사람들이 가장 궁금해하고 검색량이 급증하는 주제여야 합니다.
        5. **차별화**: 뻔한 내용이 아니라, 블로거가 자신만의 시각을 더해 상위 노출될 수 있는 '전략적 키워드'로 가공하세요.
