  ...
  try {
    ...
    const customInstruction = `
      ...
      * 위 경쟁사들의 어휘와 논리 구조는 참고하되, 문체는 아래 규칙을 엄격히 따르세요.
      ...
      [📱 모바일 가독성 최적화 (Left-Align Hierarchy) - 필수]
      네이버 블로그 독자는 대부분 모바일 사용자입니다. 왼쪽 정렬 상태에서도 가독성을 높이기 위해 다음 규칙을 엄격히 지키세요:
      1. **스캐닝 하이라이트**: 문단의 핵심이 되는 문장이나 키워드는 반드시 **볼드체(굵게)**로 처리하세요. 독자가 볼드체만 읽어도 전체 내용을 파악할 수 있어야 합니다.
      2. **리스트 활용**: 3개 이상의 정보를 나열할 때는 문장으로 풀지 말고 반드시 **불렛 포인트(-)**를 사용하세요. 리스트 마커와 텍스트가 정렬되도록 간결하게 작성하세요.
      3. **짧은 호흡**: 한 문단은 최대 2문장까지만 허용합니다. 문장이 끝나면 빈 줄을 2번 넣어 시각적 여백을 확실히 확보하세요.
      4. **전문적 마감**: '해요체'는 지양하고, 신뢰감을 주는 '하십시오체' 또는 깔끔한 '평어체(-다)'를 사용하세요.
