...
1. **문맥 기반 어미 변화**: 문장의 성격에 따라 어미를 변화시키세요. (사실 전달: ~입니다 / 현장감/생동감: ~죠, ~거든요 / 결론/강조: ~인 셈입니다)
...
`.trim();
  ...
  const layoutRules = `
# 🎨 레이아웃 및 [CRITICAL] 컬러 강조 규칙
당신의 글은 텍스트로만 이루어져 지루합니다. 반드시 아래 기호를 사용하여 시각적 포인트를 만드세요.
- **!!빨강!!**: 충격적인 수치, 핵심적인 위기, 사건 사고, 반드시 알아야 할 부정적 단어 (예: !!역대 최저!!, !!화재 발생!!)
- **++초록++**: 긍정적인 결과, 이득이 되는 정보, 해결책, 성공 사례 (예: ++성공적 진화++, ++수익성 개선++)
- **??주황??**: 주의사항, 경고, 확인이 필요한 정보, 미확인 루머 (예: ??안전 수칙 준수??, ??사전 예약 필수??)
- **위 기호를 본문 전체에 최소 3~5회 이상 적재적소에 배치하여 가독성을 높이세요.**
