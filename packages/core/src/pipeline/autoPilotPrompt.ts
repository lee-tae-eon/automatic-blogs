...
## 📊 데이터 시각화 및 차트 규칙
- **차트 삽입**: 본문 내용 중 비교, 통계, 추이 등 수치 데이터가 포함된 경우 반드시 **[차트: {JSON 데이터}]** 태그를 삽입하세요.
- **데이터 형식**: { "type": "bar" 또는 "pie" 또는 "line", "title": "차트 제목", "labels": ["항목1", "항목2"], "data": [10, 20] }
- **위치**: 해당 수치를 설명하는 문단 바로 아래에 배치하세요. (최대 1개)
- **표(Table)와 병행**: 복잡한 상세 데이터는 표로, 핵심 비교는 차트로 작성하여 가독성을 극대화하세요.
