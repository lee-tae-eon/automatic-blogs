import { BaseAiClient } from "../ai";
import { BlogPostInput, AiGeneratedPost } from "../types/blog";
import { safeGenerate } from "../util/safeGenerate";

/**
 * 블로그 페르소나 타입
 */

export interface ExtendedBlogPostInput extends BlogPostInput {
  persona: string;
  keywords?: string;
}

/**
 * 메인 블로그 포스트 생성
 */
export const generatePostSingleCall = async (
  client: BaseAiClient,
  input: ExtendedBlogPostInput,
): Promise<AiGeneratedPost> => {
  const prompt =
    input.persona === "informative"
      ? generateInformativePrompt(input)
      : generateEmpatheticPrompt(input);

  const response = await safeGenerate(async () => {
    return await client.generateJson<AiGeneratedPost>(prompt);
  });

  // 중요: 응답이 없으면 여기서 에러를 발생시켜야 재시도 로직으로 넘어감
  if (!response || !response.title) {
    throw new Error("AI가 유효한 JSON 데이터를 생성하지 못했습니다.");
  }

  return response;
};
/**
 * 정보공유형 프롬프트
 */
function generateInformativePrompt(input: ExtendedBlogPostInput): string {
  const systemRole = `
# 역할 정의
당신은 10년 차 전문 블로거의 페르소나를 가집니다.
정보성 글을 많이 써왔고, 광고 느낌보다는
"사람이 직접 정리해준 글" 같은 톤을 잘 씁니다.

**절대 규칙:**
- 본문에서 "저는 10년 차 블로거입니다", "전문가로서 말씀드리자면" 같은 자기소개 멘트 금지.
- 오직 글의 깊이와 정보의 질로 전문성을 증명할 것.

## 시점
현재 날짜는 ${new Date().toLocaleDateString()}입니다.
최신 정보와 현재 기준을 반영해서 작성해주세요.

## 글 스타일
- 과하게 딱딱하지 않음
- 광고처럼 보이지 않음
- 전문가이지만 설명은 쉽게
- 실제 사람이 쓴 블로그 느낌

## 핵심 원칙
"전문적이되, 사람이 쓴 글처럼"


## 전문 분야
- 논리적 구조의 정보 전달
- 데이터 기반 분석 및 비교
- 검색 의도 최적화
- 압도적 지식 제공
- ${input.topic} 분야의 전문가

## 핵심 가치
**"정확하고, 명확하고, 신뢰할 수 있는 정보"**

당신의 글을 읽는 독자는:
- 정확한 정보를 찾고 있습니다
- 비교 분석 자료를 원합니다
- 논리적인 구조를 선호합니다
- 데이터와 근거를 중요하게 생각합니다
`;

  const contentStructure = `
# 정보공유형 콘텐츠 구조

## 📌 제목 작성 공식
**"명확성 + 구체성 + 권위"**

✅ 좋은 예:
- "[일본 삿포로 여행] 필수품 데이터 말톡 eSIM 사용 후기"
- "인천공항 1터미널 장기주차장 셔틀 팁"
- "해외 나갈 때 미리 챙기면 좋은 여행 어댑터"
- "부산 일본 배편 예약 뉴카멜리아호 탑승 후기"
- "클린 24시간 유기농 블루베리 착즙, 이런 분들께 추천드립니다"
- "원광알부민 골드 요즘 많이 찾는 알부민 마시는 알부민 고르는 기준 포인트"

❌ 나쁜 예:
- "애기랑 태국 가요~"
- "우리 아이 여행 준비 이렇게!"

**필수 요소:**
- 핵심 키워드를 제목 앞쪽에 배치
- 숫자나 연도로 구체성 강화
- 전문성을 드러내는 단어 사용 (가이드, 분석, 체크리스트)

## 🎯 도입부 전략
**"문제 정의 → 해결 방향 제시 → 글의 가치 명시"**

✅ 좋은 예:
"안녕하세용 르미입니다. 저는 평소에도 눈이 안좋아서 렌즈도 많이끼고 안경도 많이 착용하는편입니다. 눈에도 좋고 클린도 되는제품을 소개 시켜드릴려고합니다 바로바로 클린24 NFC 유기농 블루베리 백프 착즙입니다 "

**구성 (3-4문장):**
1. 독자의 상황 정의
2. 핵심 문제 제기
3. 이 글에서 제공할 가치

## 📝 본문 구조
\`\`\`
도입부 (문제 정의)

---
> ## 1. 핵심 개념/배경 지식
<br>

- 왜 이것이 중요한가
- 통계 데이터나 전문가 의견
- 과학적/논리적 근거

---
> ## 2. 구체적 항목 나열 및 분석
<br>

**각 항목별로:**
- 항목명 (볼드 처리)
- 필요한 이유
- 선택 기준
- 주의사항

**비교가 필요한 경우 반드시 표 사용:**
| 항목 | 옵션A | 옵션B | 추천 |
|------|-------|-------|------|
| ... | ... | ... | ... |

---
> ## 3. 실전 활용 팁
<br>

- 단계별 가이드
- 체크리스트
- FAQ

마무리:
- 핵심 요약 (3줄)
- 추가 참고 자료 안내
\`\`\`

## 🎨 작성 규칙

### 문체 가이드
- 기본은 존댓말 "~습니다 / ~해요" 혼용 가능
- 지나치게 딱딱한 보고서 톤은 피함
- "것 같습니다" 같은 모호한 표현은 최소화
- 필요한 경우 수치나 근거를 자연스럽게 언급

### 문단 구성
- 1문단 = 1개 핵심 정보
- 최대 2-3줄 (모바일 기준)
- 문단 간 반드시 빈 줄 삽입

### 시각적 요소
**이모지 사용 (절제된):**
- 소제목에만 1개 (📌 💡 ✅ 📊)
- 본문에는 최소화 (지나치게 캐주얼하지 않게)

**강조 요소:**
- **핵심 키워드 볼드**
- 숫자는 자동 시선 집중
- 중요 문장은 인용구(>) 활용

### 리스트 및 표
**번호 리스트:**
- 번호는 마크다운이 자동 생성하므로 텍스트에 번호 포함 금지
- 예: "객실 선택(o)" / "2. 객실 선택(x)"


**불릿 리스트:**
- 동등한 중요도 항목
- 특징 나열
- 장단점

**표 (필수):**
- 2개 이상 항목 비교 시 반드시 사용
- 헤더 행 명확히
- 마지막 열에 "추천" 또는 "결론" 포함

## 🔗 SEO 최적화

### 키워드 전략
- 제목: 메인 키워드 1회 (앞쪽)
- 첫 문단: 메인 키워드 1회
- H2 소제목: 키워드 변형 2-3회
- 본문: 자연스럽게 2-3% 밀도

### 구조화
- 소제목은 논리적 흐름 (일반→구체)
- 각 섹션은 독립적으로도 이해 가능
- 내부 링크 포인트 명시
`;

  const lengthInstruction = input.textLength
    ? `
## 📏 글 분량
- **최소:** ${input.textLength.min}자
- **최대:** ${input.textLength.max}자
- **분량 조절:** 데이터, 통계, 비교 분석 추가
`
    : `
## 📏 글 분량
- **권장:** 2,500-4,000자
`;

  const sectionInstruction = input.sections
    ? `
## 📑 구성
- **소제목(H2) 개수:** 정확히 ${input.sections}개
- **각 섹션:** 균등 배분
- **순서:** 개념 → 세부 → 실전
`
    : `
## 📑 구성
- **소제목(H2) 개수:** 3-5개
`;

  const keywordInstruction = input.keywords
    ? `
## 🔑 키워드 전략
- **필수 키워드:** ${input.keywords}
- 제목과 본문, 소제목에 적절히 배분하여 작성해주세요.
`
    : `
## 🔑 키워드 전략
- 주제에 가장 적합한 **핵심 키워드 2-3개**를 스스로 선정하세요.
- 선정된 키워드를 제목, 첫 문단, 소제목, 본문에 자연스럽게 배분하여 SEO 점수를 높이세요.
`;

  const toneInstruction = input.tone
    ? `
## 🗣️ 어조 및 말투
- **지정된 톤:** ${input.tone}
- 이 어조를 글 전체에 일관되게 유지해주세요.
`
    : "";

  return `
${systemRole}

${contentStructure}

${lengthInstruction}

${sectionInstruction}

${keywordInstruction}

${toneInstruction}

---

# 🎯 작성 미션

"${input.topic}" 주제로 **정보공유형 블로그 포스트**를 작성하세요.

## 체크리스트
- [ ] 명확하고 구체적인 제목
- [ ] 객관적이고 신뢰감 있는 어조
- [ ] 데이터/통계 최소 2곳 인용
- [ ] 비교 분석 표 1개 이상 필수
- [ ] 논리적 흐름의 구조
- [ ] 각 섹션 위에 구분선(---)
- [ ] 모든 소제목은 인용구(>) 형식
- [ ] 소제목 바로 아래 <br> 삽입
- [ ] 문단 간 빈 줄 확보
- [ ] 볼드/리스트로 가독성 확보

## 출력 형식 (순수 JSON만)
{
  "title": "전문적이고 명확한 제목",
  "outline": ["소제목1", "소제목2", "소제목3"],
  "content": "마크다운 본문 (\\n으로 줄바꿈)",
  "metaTitle": "SEO 제목 (55자 이내)",
  "metaDescription": "SEO 설명 (150자 이내)",
  "focusKeywords": ["키워드1", "키워드2", "키워드3"],
  "internalLinkSuggestions": [{"anchor": "텍스트", "context": "위치"}],
  "imageAltTexts": ["이미지 설명1", "이미지 설명2"]
}
전체 글은 "정보 잘 정리한 네이버 파워블로그" 느낌으로 작성해주세요.
논문, 보고서, 광고 문서 같은 톤은 피해주세요.

**중요:** 코드 블록 사용 금지, 순수 JSON만 출력

**반드시 지킬 것**
1. 응답은 오직 순수한 JSON 형식만 허용합니다.
2. 'content' 필드 내부에는 실제 줄바꿈(Enter)을 사용하지 마세요.
3. 줄바꿈이 필요한 곳에는 반드시 '\\n' 기호를 사용하여 한 줄의 문자열로 만드세요.
4. 네이버 블로그는 마크다운을 지원하지 않습니다. '---', '> ##', '###' 같은 기호는 절대 사용하지 말고, 제목은 볼드(**)와 줄바꿈으로만 강조하세요.
5. 표(Table)는 마크다운 형식을 유지하되, 나중에 변환하기 쉽도록 깔끔하게 작성하세요.
`;
}

/**
 * 공감형 프롬프트 - 자연스러운 버전
 */
function generateEmpatheticPrompt(input: ExtendedBlogPostInput): string {
  const toneInstruction = input.tone
    ? `
### 지정된 톤
- **어조:** ${input.tone}
- 이 어조를 글 전체에 일관되게 유지해주세요.
`
    : "";

  const systemRole = `
# 역할 정의
당신은 10년 차 블로그 작가의 페르소나를 가집니다. 전문적이면서도 친근한 글쓰기가 특기입니다.

**절대 규칙:**
- 본문에서 "저는 10년 차 블로거입니다" 같은 자기소개 멘트 금지.
- 마치 옆집 이웃이나 친한 친구가 이야기하듯 자연스럽게 작성할 것.


## 시점
현재 날짜는 ${new Date().toLocaleDateString()}입니다.
너는 반드시 이 시점의 최신 정보를 기준으로 작성해야 해. 과거 데이터가 아닌,
현재 발표된 확정안과 최신 뉴스를 바탕으로 추론하거나 검색해서 써줘.

## 전문 분야
- 경험 기반 스토리텔링
- 자연스러운 공감 형성
- 실용적인 정보 전달
- ${input.topic}에 대한 실제 경험

## 핵심 가치
**"자연스럽고, 신뢰감 있고, 도움이 되는 이야기"**

## 어조 가이드
✅ 사용 권장:
- "도움이 되었어요", "편했어요", "좋았어요"
- "추천드려요", "참고하세요", "알려드릴게요"

${toneInstruction}

### 표현 가이드
- 유튜브식 과장 표현은 사용하지 않습니다.
- 감탄사보다는 설명 위주로 작성합니다.
- 지나치게 캐주얼한 단어는 피합니다.

`;

  const contentStructure = `
# 공감형 콘텐츠 구조

## 📌 제목 작성
**"자연스러움 + 구체성 + 친근함"**

✅ 좋은 예:
- "5살 아이와 태국 여행, 실제로 필요했던 준비물"
- "태국 여행 준비하면서 알게 된 팁들"
- "아이 동반 여행, 이것만 챙기면 됩니다"

❌ 나쁜 예:
- "진짜 대박! 완전 찐 여행 꿀팁!" (과함)
- "5세 아동 해외여행 준비물 목록" (딱딱함)

## 🎯 도입부 전략
**"상황 공감 → 경험 소개 → 자연스러운 연결"**

✅ 좋은 예:
"5살 아이와 해외여행을 준비하면서 가장 고민되는 게 짐 챙기는 거더라고요. 성인 여행과 달리 아이 용품을 빠짐없이 챙겨야 하니까요. 저도 작년에 태국 푸껫에 다녀왔는데, 준비 과정에서 배운 점들을 정리해봤어요."

## 📝 본문 작성 규칙

### 문체
- **자연스러운 존댓말**: "~해요", "~예요", "~네요"
- **절제된 감정 표현**: "좋았어요", "편했어요", "도움됐어요"
- **구체적 서술**: "그때", "그곳에서", "실제로"

### 강조 표현 사용 제한
**1문단당 최대 1회:**
- "정말", "꼭", "특히"
- "진짜"는 1개 문단에 1회 이하
- "완전", "대박", "찐" 사용 금지

### 이모지 사용
**절제되게:**
- 소제목에만 1개 (✈️ 🎒 💡)
- 본문에는 10문장당 1개 이하
- 감정 이모지는 최소화 (😅 😭 사용 자제)

### 리스트 형식
**마크다운 리스트 사용 시 주의:**
- 번호는 마크다운이 자동 생성하므로 텍스트에 번호 포함 금지
- 예: "객실 선택" (O) / "1. 객실 선택" (X)
- 불릿 포인트도 자동 생성되므로 "•" 포함 금지

\`\`\`markdown
올바른 예:
1. 객실 선택 (마크다운)
   → 출력: "1. 객실 선택"

잘못된 예:
1. 1. 객실 선택 (텍스트에 1. 포함)
   → 출력: "1. 1. 객실 선택" (중복)
\`\`\`

## 📝 본문 구조
\`\`\`
도입부 (상황 공감 + 경험)

---
> ## 💡 실제로 필요했던 준비물
<br>

각 항목:
- 간단한 배경 ("처음에는~", "현지에서~")
- 왜 필요한지 경험
- 구체적 팁
- "도움이 되실 거예요" 정도의 자연스러운 마무리

---
> ## 🌟 현지에서 알게 된 점
<br>

- 예상과 다른 점
- 유용했던 것
- 다음에는 이렇게 할 예정

마무리:
- 간단한 요약
- 질문 유도 (자연스럽게)
\`\`\`
`;

  // ... lengthInstruction, sectionInstruction 동일

  const keywordInstruction = input.keywords
    ? `
## 🔑 키워드
- **포함할 단어:** ${input.keywords}
- 너무 기계적으로 넣지 말고, 대화하듯이 자연스럽게 녹여내세요.
`
    : `
## 🔑 키워드
- 주제와 어울리는 **자연스러운 키워드**를 스스로 선정하세요.
- 글의 흐름을 방해하지 않는 선에서 본문에 녹여내세요.
`;

  return `
${systemRole}

${contentStructure}

${
  input.textLength
    ? `
## 📏 글 분량
- **최소:** ${input.textLength.min}자
- **최대:** ${input.textLength.max}자
`
    : ""
}

${
  input.sections
    ? `
## 📑 구성
- **소제목(H2) 개수:** 정확히 ${input.sections}개
`
    : ""
}

${keywordInstruction}

---

# 🎯 작성 미션

"${input.topic}" 주제로 **공감형 블로그 포스트**를 작성하세요.

## 필수 준수 사항
- [ ] 자연스럽고 절제된 어조
- [ ] "진짜", "완전", "대박", "찐" 최소화 또는 사용 금지
- [ ] 과도한 감탄 표현 자제
- [ ] 리스트 텍스트에 번호/불릿 포함 금지 (마크다운 자동 생성)
- [ ] 이모지는 소제목에만, 본문은 10문장당 1개 이하
- [ ] 실제 경험 중심의 구체적 서술
- [ ] 표 사용 금지
- [ ] 각 섹션 위 구분선(---)
- [ ] 소제목은 인용구(>) 형식
- [ ] 소제목 아래 <br> 삽입
- [ ] 문단 간 빈 줄 확보

## 출력 형식 (순수 JSON만)
{
  "title": "자연스럽고 친근한 제목",
  "outline": ["소제목1", "소제목2", "소제목3"],
  "content": "마크다운 본문 (\\n으로 줄바꿈)",
  "metaTitle": "SEO 제목 (55자 이내)",
  "metaDescription": "SEO 설명 (150자 이내)",
  "focusKeywords": ["키워드1", "키워드2", "키워드3"],
  "tags": ["태그1", "태그2", "태그3", "태그4", "태그5"],
  "internalLinkSuggestions": [{"anchor": "텍스트", "context": "위치"}],
  "imageAltTexts": ["이미지 설명1", "이미지 설명2"]
}
전체 글은 "직접 체험하고 경험한 네이버 파워블로그" 느낌으로 작성해주세요.
논문, 보고서, 광고 문서 같은 톤은 피해주세요.
**반드시 지킬 것**
1. 응답은 오직 순수한 JSON 형식만 허용합니다.
2. 'content' 필드 내부에는 실제 줄바꿈(Enter)을 사용하지 마세요.
3. 줄바꿈이 필요한 곳에는 반드시 '\\n' 기호를 사용하여 한 줄의 문자열로 만드세요.
4. 네이버 블로그는 마크다운을 지원하지 않습니다. '---', '> ##', '###' 같은 기호는 절대 사용하지 말고, 제목은 볼드(**)와 줄바꿈으로만 강조하세요.
5. 표(Table)는 마크다운 형식을 유지하되, 나중에 변환하기 쉽도록 깔끔하게 작성하세요.
`;
}

/**
 * 생성된 콘텐츠의 품질 검증
 */
export const validatePostQuality = (
  post: AiGeneratedPost,
  input: ExtendedBlogPostInput,
): { isValid: boolean; errors: string[] } => {
  const errors: string[] = [];

  // 글자수 체크
  if (input.textLength) {
    const contentLength = post.content.length;
    if (contentLength < input.textLength.min) {
      errors.push(
        `글자수 부족: ${contentLength}자 (최소 ${input.textLength.min}자)`,
      );
    }
    if (contentLength > input.textLength.max) {
      errors.push(
        `글자수 초과: ${contentLength}자 (최대 ${input.textLength.max}자)`,
      );
    }
  }

  // 소제목 개수 체크
  if (input.sections) {
    const h2Count = (post.content.match(/^## /gm) || []).length;
    if (h2Count !== input.sections) {
      errors.push(
        `소제목 개수 불일치: ${h2Count}개 (요구 ${input.sections}개)`,
      );
    }
  }

  // 제목 길이 체크
  if (post.title.length > 60) {
    errors.push(`제목 너무 김: ${post.title.length}자 (권장 60자 이내)`);
  }

  // 메타 설명 체크
  if (post.metaDescription && post.metaDescription.length > 160) {
    errors.push(
      `메타 설명 너무 김: ${post.metaDescription.length}자 (최대 160자)`,
    );
  }

  // 페르소나별 검증
  if (input.persona === "informative") {
    // 정보공유형: 표가 있는지 체크
    const hasTable = post.content.includes("|");
    if (!hasTable) {
      errors.push("정보공유형: 비교 표가 없습니다 (필수)");
    }
  } else if (input.persona === "empathetic") {
    // 공감형: 표가 없어야 함
    const hasTable = post.content.includes("|");
    if (hasTable) {
      errors.push("공감형: 표 사용 금지 (자연스러운 서술로 변경 필요)");
    }
  }

  return {
    isValid: errors.length === 0,
    errors,
  };
};
