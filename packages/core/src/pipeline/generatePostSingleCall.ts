import { BaseAiClient } from "../ai";
import { BlogPostInput, AiGeneratedPost } from "../types/blog";
import { safeGenerate } from "../util/safeGenerate";

/**
 * 블로그 페르소나 타입 확장
 */
export interface ExtendedBlogPostInput extends BlogPostInput {
  persona: string;
  keywords?: string;
}

/**
 * 메인 블로그 포스트 생성 함수
 */
export const generatePostSingleCall = async (
  client: BaseAiClient,
  input: ExtendedBlogPostInput,
): Promise<AiGeneratedPost> => {
  const prompt =
    input.persona === "informative"
      ? generateInformativePrompt(input)
      : generateEmpatheticPrompt(input);

  const response = await safeGenerate(async () => {
    return await client.generateJson<AiGeneratedPost>(prompt);
  });

  if (!response || !response.title) {
    throw new Error("AI가 유효한 JSON 데이터를 생성하지 못했습니다.");
  }

  return response;
};

/**
 * 정보공유형 프롬프트 생성
 */
function generateInformativePrompt(input: ExtendedBlogPostInput): string {
  const systemRole = `
# 역할 정의
당신은 최신 정보를 잘 정리하여 핵심내용을 전달하는 전문 블로거입니다. 정보의 깊이와 신뢰성을 바탕으로 사람이 직접 정리한 듯한 글을 씁니다.
- 시점: 현재 날짜는 ${new Date().toLocaleDateString()}입니다. 최신 정보를 반영하세요. 날짜를 반영하는게 아닙니다.
- 전문 분야: 논리적 구조, 데이터 기반 분석, ${input.topic} 전문가.
- 핵심 원칙: "글의 내용은 핵심을 기반으로 사람이 읽기 좋게(가독성) 잘 세분화 하여 정리하여야함.
`;

  const contentStructure = `
# 콘텐츠 구조 및 스타일 가이드

## 📝 소제목 및 문단 구성 규칙 (필수)
1. **소제목 개수**: 반드시 **5개~6개**의 소제목 섹션을 작성하세요.
2. **소제목 형식**: 모든 소제목은 반드시 \`> ## 소제목명\` 형식을 사용하세요.
   - 이 형식은 네이버 에디터에서 '인용구(따옴표)' 블록으로 변환되어 중앙 정렬 효과를 줍니다.
3. **문장 길이 제한**: 각 소제목 아래의 본문 문장은 **200자 이하**로 짧게 작성하세요. 호흡이 길어지지 않게 주의하세요.
4. **문단 구성**: 2~3문장마다 빈 줄을 삽입하여 모바일 가독성을 극대화하세요.

## 📝 본문 레이아웃 예시
\`\`\`markdown
도입부 (인사말 및 배경 설명)

---
> ## 1. 첫 번째 섹션 제목
(공백 한 줄)
(공백 한 줄)
(공백 한 줄)

(200자 이내의 간결한 설명 문단)

---
> ## 2. 두 번째 섹션 제목
(공백 한 줄)
(공백 한 줄)

(비교 표 또는 상세 리스트 포함)
... (총 5개의 소제목 섹션 반복)
\`\`\`
`;

  const commonInstructions = `
## 🎨 시각적 요소 및 SEO
- **이모지**: 소제목에 1개씩만 사용 (📌, 💡 등). 본문엔 최소화.
- **표(Table)**: 정보공유형이므로 비교 분석 표를 반드시 1개 이상 포함하세요.
- **강조**: 핵심 키워드는 **볼드처리** 및 강조 색상을 주세요.
- **구분선**: 각 소제목 섹션 상단에 \`--- \`를 삽입하세요.
`;

  return `
${systemRole}
${contentStructure}
${commonInstructions}

# 🎯 작성 미션
"${input.topic}" 주제로 정보공유형 포스트를 작성하세요.

## 체크리스트
- [ ] 소제목은 반드시 **5개**이며 \`> ## \` 형식을 사용할 것.
- [ ] 각 문장은 **200자 이내**로 작성할 것.
- [ ] 섹션 사이마다 구분선(---)을 넣을 것.
- [ ] 비교 분석 표를 1개 이상 포함할 것.

## 출력 형식 (순수 JSON만)
{
  "title": "제목",
  "outline": ["소제목1", "소제목2", "소제목3", "소제목4", "소제목5"],
  "content": "본문 내용 (\\n으로 줄바꿈)",
  "metaTitle": "SEO 제목",
  "metaDescription": "SEO 설명",
  "focusKeywords": ["키워드1", "키워드2"]
}

**반드시 지킬 것**
1. 응답은 오직 순수한 JSON 형식만 허용합니다.
2. 'content' 내부에 실제 줄바꿈을 사용하지 말고 반드시 '\\\\n' 기호를 사용하세요.
3. 네이버 블로그는 마크다운을 지원하지 않으므로 \`> ## \` 외의 특수 기호(### 등)는 삼가세요.
`;
}

/**
 * 공감형 프롬프트 생성
 */
function generateEmpatheticPrompt(input: ExtendedBlogPostInput): string {
  const systemRole = `
# 역할 정의
당신은 직접 경험하고 느낌점을 잘 표현하는 체험형 블로거입니다. 내가 직접 체험한 자연스러운 경험 기반 스토리텔링을 구사합니다.
- 시점: 현재 날짜 ${new Date().toLocaleDateString()} 기준 최신 정보를 반영하세요. 날짜를 반영하는게 아닙니다.
- 핵심 원칙: 전문가라는 느낌을 주면 안됩니다. 단 공감하고 경험한 내용을 수필 형식으로 작성합니다.
`;

  return `
${systemRole}

# 콘텐츠 구조 가이드
1. **소제목(H2)**: 반드시 **5개 ~ 6개**를 작성하세요.
2. **소제목 형식**: 반드시 \`> ## 소제목명\` 형식을 사용하여 인용구 스타일을 적용하세요.
3. **문장 길이**: 모바일 가독성을 위해 모든 문장은 **200자 이하**로 작성하세요.
4. **어조**: "~했어요", "~네요" 등 친근한 말투를 사용하세요.

# 🎯 작성 미션
"${input.topic}" 주제로 공감형 포스트를 작성하세요.

## 필수 준수 사항
- [ ] 소제목 5~6개 (\`> ## \` 형식)
- [ ] 각 문장 200자 이내 작성
- [ ] 실제 경험이 묻어나는 자연스러운 서술
- [ ] 표 사용 금지 (공감형은 텍스트 중심)

## 출력 형식 (순수 JSON만)
{
  "title": "친근한 제목",
  "outline": ["소제목1", "소제목2", "소제목3", "소제목4", "소제목5"],
  "content": "본문 내용 (\\n으로 줄바꿈)",
  "tags": ["태그1", "태그2"]
}

**반드시 지킬 것**
1. 응답은 오직 순수한 JSON 형식만 허용합니다.
2. 'content' 내부에 '\\n' 기호로 줄바꿈을 처리하세요.
3. 소제목 인용구 문법(\`> ## \`)을 철저히 지키세요.
`;
}

/**
 * 생성된 콘텐츠 품질 검증 로직
 */
export const validatePostQuality = (
  post: AiGeneratedPost,
  input: ExtendedBlogPostInput,
): { isValid: boolean; errors: string[] } => {
  const errors: string[] = [];

  // 1. 소제목 개수 및 형식 검증 (5개 고정 및 인용구 문법 체크)
  const quoteH2Count = (post.content.match(/^> ## /gm) || []).length;
  if (quoteH2Count !== 5) {
    errors.push(`소제목 개수 오류: ${quoteH2Count}개 발견 (반드시 5개여야 함)`);
  }

  // 2. 문장 길이 검증 (개별 문장이 200자 이하인지 체크)
  const sentences = post.content.split(/[.!?]\s|\n/);
  for (const s of sentences) {
    if (s.trim().length > 200) {
      errors.push(`문장 길이 초과: 200자를 넘는 문장이 발견되었습니다.`);
      break;
    }
  }

  // 3. 페르소나별 필수 요소 검증
  if (input.persona === "informative") {
    const hasTable = post.content.includes("|");
    if (!hasTable) errors.push("정보공유형: 비교 표가 없습니다.");
  } else if (input.persona === "empathetic") {
    const hasTable = post.content.includes("|");
    if (hasTable) errors.push("공감형: 표를 제거하고 서술형으로 작성하세요.");
  }

  return {
    isValid: errors.length === 0,
    errors,
  };
};
