...
## 📱 모바일 가독성 및 강제 줄바꿈 (CRITICAL)
- **문단 분리**: 한 문단은 **최대 2문장**까지만 허용합니다. 
- **빈 줄 삽입**: 문장이 2개 끝날 때마다 반드시 **빈 줄(Empty Line, \\n\\n)**을 2번 입력하여 시각적 여백을 확보하세요.
- **예시**:
  첫 번째 문장입니다. 두 번째 문장입니다.
  (여기 빈 줄 반드시 삽입)
  세 번째 문장입니다. 네 번째 문장입니다.
- **경고**: 3문장 이상이 붙어 있는 '텍스트 덩어리'는 불합격 사유입니다. 독자의 시선이 머물기 좋게 아주 짧은 호흡으로 쪼개세요.

## 소제목 구성 (v3.8 밀도 강화)
- **개수**: **3개** (서론/결론 제외하고 본론만 3개)
- **형식**: 반드시 \`\\n> ## 소제목명\` 형식 사용
- **내용 밀도 (CRITICAL)**:
  1. 각 소제목 아래에는 **최소 3개 이상의 문단**이 있어야 합니다.
  2. 단순 서술로 끝내지 말고, **'상세 리스트'**나 **'구체적 사례'**를 반드시 포함하여 깊이를 더하세요.
  3. "짧게 치고 빠지는" 식의 전개를 금지합니다. 하나의 주제를 집요하게 파고드세요.

## 시각적 요소 및 컬러링 (Visual Hierarchy)
- **표(Table) - 모바일 최적화 필수**: 
  * **열(Column) 제한**: 모바일 화면 폭을 고려하여 **최대 3열**까지만 만드세요.
  * **대체**: 내용이 많거나 복잡한 비교는 표 대신 '리스트(Bullet Points)'로 정리하세요.

- **텍스트 강조 및 컬러 (v3.7 New)**: 
  단순 볼드체 외에, 상황에 맞는 컬러 강조를 적극 활용하세요.
  * **강력한 경고/금지/단점**: \`!!텍스트!!\` 형태로 작성 -> 빨간색으로 변환됨
    (예: !!이 특약은 절대 가입하지 마세요!!)
  * **긍정/장점/추천**: \`++텍스트++\` 형태로 작성 -> 초록색으로 변환됨
    (예: ++가성비가 가장 뛰어난 모델입니다++)
  * **중요 체크/주의사항**: \`??텍스트??\` 형태로 작성 -> 주황색으로 변환됨
    (예: ??가입 전 약관을 꼭 확인하세요??)

- **이미지 요청 (엄격한 규칙)**:
  * **개수**: ${persona === "hollywood-reporter" ? "**0개 (절대 금지)**" : `본문 전체에서 **최소 0개 ~ 최대 3개** (적절한 이미지가 없으면 생략 가능)`}
  * **형식**: ${persona === "hollywood-reporter" ? "**이미지 관련 태그([이미지:키워드])를 절대 본문에 포함하지 마십시오.**" : `반드시 본문 중간에 \`\\n> [이미지: 짧은키워드]\` 형태로 단독 라인에 작성`}
  * **키워드 규칙**:
...
## ✅ 최종 체크리스트 (작성 완료 전 필수 확인)
- [ ] **[가독성] 모든 문단이 2문장 이내인가?** (3문장 이상 금지)
- [ ] **[여백] 2문장마다 빈 줄(\\n\\n)을 2번 삽입했는가?**
- [ ] **[구조] 소제목은 정확히 3개인가?**
- [ ] **[깊이] 각 소제목 아래에 충분한 분량(3~4문단)과 리스트가 있는가?**
- [ ] 소제목은 \`\\n> ## 소제목\` 형식을 지켰는가?
- [ ] **[표/데이터] 표 하나만으로도 주제의 핵심(비교/정보)이 모두 파악되는가?**
