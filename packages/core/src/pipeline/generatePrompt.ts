...
## 가독성 및 줄바꿈 (Mobile First - CRITICAL)
- **문단 길이 제한**: 한 문단은 **최대 2~3문장**을 절대 넘지 마세요.
- **줄바꿈 규칙**: 2~3문장마다 반드시 엔터(줄바꿈)를 두 번 입력하여 빈 줄(시각적 여백)을 만드세요.
- **경고**: 텍스트가 뭉쳐 있으면(4문장 이상) 독자가 바로 이탈합니다. **블로그 화면 기준 4줄 이상** 넘어가는 문단은 절대 금지입니다.
- **호흡**: 아주 짧은 호흡으로 끊어서 작성하세요.
...
## 시각적 요소
- **표(Table) - CRITICAL STRATEGY**: 
  * **필수성**: ${metrics.tableRequired ? "**필수** (포스팅의 핵심 내용을 담은 비교/분석표)" : "선택 (데이터 요약이 필요할 때 반드시 사용)"}
  * **정보 밀도**: "표만 봐도 글 전체 내용을 이해할 수 있을 정도"로 구체적이고 정확한 수치/정보를 입력하세요.
  * **구조**: 단순히 2열이 아니라, '구분/항목', '상세 내용', '비고/특징' 등 최소 3열 이상의 다각도 분석 표를 지향하세요.
  * **가독성**: 마크다운 표 형식(\`|---|---|\`)을 정확히 지키고, 셀 안의 내용이 너무 길면 줄바꿈(\`<br>\`)을 적절히 사용하여 라인이 깨지지 않게 하세요.
...
- [ ] **[가독성] 모든 문단이 2~3문장 이내인가?** (블로그 기준 4줄 초과 금지)
- [ ] **[여백] 문단 사이마다 빈 줄(Empty Line)을 삽입했는가?**
