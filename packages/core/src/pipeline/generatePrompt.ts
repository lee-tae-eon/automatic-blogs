...
## 🔗 출처 표기 규칙 (맥락에 따라 선택)
1. 제공된 뉴스 데이터가 주제(시사, 트렌드, 사건 등)와 **밀접하게 연관될 때만** 본문에 인용하고 JSON 응답의 'references' 필드에 포함하세요.
2. 만약 주제가 **단순 가이드, 구매처 안내, 일반 리뷰** 등 최신 뉴스가 굳이 필요 없는 경우라면, 뉴스 데이터를 무시하고 출처를 기재하지 않아도 됩니다.
3. 뉴스를 인용했다면 출처 표기는 필수이며, 형식은 [{"name": "매체명", "url": "http://..."}]을 지켜주세요.
4. 절대 관련 없는 뉴스를 억지로 끼워 넣지 마세요.`;
  }
...
- **🚨 사실성 엄수 (CRITICAL)**: 장소, 가게 이름, 주소, 가격 등 모든 고유 명사는 **반드시 실제 존재하는 정보**여야 합니다. 
  ...
  * 제공된 뉴스나 검색 데이터에 없는 정보를 추측해서 쓰지 마십시오.
