...
3. **[Dwell-Time] '나의 해당 여부' 체크리스트 의무화**:
   - 본문 중간이나 하단에 독자가 직접 자신의 상황을 체크해 볼 수 있는 **마크다운 리스트 형태의 체크리스트**를 반드시 포함하세요.
...
5. **최소 본문 길이**: content 필드의 본문은 반드시 **한글 기준 2,000자 이상**이어야 합니다. 짧은 글은 검색 노출에 불리합니다.
6. **LSI 키워드 목록 출력**: 본문에서 실제로 사용한 LSI 키워드 5~7개를 JSON의 \`lsiKeywords\` 배열에 담아 반환하세요.
`;
...
\`\`\`json
{
  ...
  "content": "마크다운 본문 (도입부 + 소제목 + [차트: {...}] + 본론 + 결론) — 최소 2000자 이상",
  "metaTitle": "SEO 제목",
