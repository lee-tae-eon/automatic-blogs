...
`;
  ...
  private async publish(page: Page, tags: string[] = [], category?: string) {
    ...
    try {
      ...
      // 6. 최종 발행 버튼 클릭 (정밀 타겟팅으로 교체)
      console.log("   최종 발행 버튼 클릭 시도...");
      let published = false;

      try {
        // ✅ 1. 레이어 안에서 정확히 "발행"이라는 글자만 있고, 현재 화면에 보이는(visible) 버튼을 찾습니다.
        const finalBtn = page
        ...
        // ✅ 2. Playwright의 클릭이 실패할 경우, 브라우저 내부에서 직접 DOM을 찾아 클릭 이벤트를 발생시킵니다.
        const jsSuccess = await page.evaluate((selector) => {
